...
static const struct gpio_dt_spec power_gpio = GPIO_DT_SPEC_GET_OR(
    DT_COMPAT_GET_ANY_STATUS_OKAY(zmk_pk_underglow_layer), power_gpios, {0});
#else
...
void pk_ug_task_render_frame_execute(void) {
  ...
  if (pk_underglow_effects[state.current_effects[active_profile_index]]
          .render) {
...
static int rgb_settings_set(const char *name, size_t len,
  ...
  if (settings_name_steq(name, "state", &next) && !next) {
    if (len != sizeof(state)) {
      LOG_WRN("Underglow state size mismatch (got %d, expected %d). Ignoring.",
              len, sizeof(state));
      return 0;
    }
...
SETTINGS_STATIC_HANDLER_DEFINE(pk_underglow, "rgb/underglow", NULL,
                               rgb_settings_set, NULL, NULL);
...
int zmk_pk_underglow_calc_effect(int direction) {
  return (state.current_effects[active_profile_index] +
          pk_underglow_effects_count + direction) %
...
int zmk_pk_underglow_select_effect(int effect) {
  ...
  LOG_INF("Selected effect: %d, layer_enabled: %d, state.on: %d", effect,
...
int zmk_pk_underglow_cycle_effect(int direction) {
  return zmk_pk_underglow_select_effect(
      zmk_pk_underglow_calc_effect(direction));
}
...
void zmk_pk_underglow_set_layer(uint8_t layer, bool wakeup) {
  LOG_INF("Setting pk underglow layer: %d. layer_enabled: %d, state.on: %d",
          layer, state.layer_enabled, state.on);
  if (!state.layer_enabled)
    return;

  const struct zmk_behavior_binding *rgbmap = pk_underglow_get_bindings(layer);
  if (rgbmap != NULL &&
      zmk_pk_underglow_apply_rgbmap(rgbmap, ZMK_KEYMAP_LEN, layer)) {
    if (!is_powered) {
      if (!wakeup && !state.on) {
...
int zmk_pk_underglow_change_spd(int direction) {
  ...
  if (state.effect_speeds[state.current_effects[active_profile_index]] == 1 &&
      direction < 0) {
    return 0;
  }
...
static struct pk_underglow_sleep_state sleep_state = {
    .is_awake = true, .rgb_state_before_sleeping = false};
...
void pk_ug_task_sync_state_execute(uint8_t layer, uint8_t state_directive) {
  ...
  uint32_t param2 =
      (state.current_effects[active_profile_index] & 0xFF) |
      ((state.effect_speeds[state.current_effects[active_profile_index]] & 0xFF)
       << 8) |
      ((layer & 0xFF) << 16) | ((state.on ? 1 : 0) << 24) |
      ((state_directive & 0x03) << 25) | ((state.layer_enabled ? 1 : 0) << 27);

  LOG_DBG("Central: Broadcasting ug_sync with layer %d, state_directive %d",
          layer, state_directive);
#if DT_HAS_COMPAT_STATUS_OKAY(zmk_behavior_pk_underglow_sync)
  struct zmk_behavior_binding binding = {
      .behavior_dev = DEVICE_DT_NAME(
          DT_COMPAT_GET_ANY_STATUS_OKAY(zmk_behavior_pk_underglow_sync)),
      .param1 = param1,
      .param2 = param2,
  };
...
static K_WORK_DELAYABLE_DEFINE(sync_peripheral_delayed_work,
                               sync_peripheral_delayed_work_handler);
...
static int pk_underglow_auto_state(bool target_wake_state) {
  ...
  LOG_DBG("Auto state changed. Target wake: %d. Syncing to peripheral...",
          target_wake_state);
...
#if IS_ENABLED(CONFIG_ZMK_SPLIT) && !IS_ENABLED(CONFIG_ZMK_SPLIT_ROLE_CENTRAL)
  LOG_DBG(
      "Peripheral: Checking if central is connected (is_central_connected=%d)",
      is_central_connected);
...
static int pk_underglow_event_listener(const zmk_event_t *eh) {
  ...
  if (as_zmk_underglow_color_changed(eh)) {
    const struct zmk_underglow_color_changed *ev =
        as_zmk_underglow_color_changed(eh);
    uint8_t layer = pk_underglow_top_layer();
    LOG_DBG("refresh layers %d, current: %d, wakeup: %d", ev->layers, layer,
            ev->wakeup);
  ...
  if (as_zmk_split_peripheral_status_changed(eh)) {
    ...
    LOG_DBG("Peripheral: Split status changed. Connected: %d",
            is_central_connected);
...
void zmk_pk_underglow_sync_state(uint32_t param1, uint32_t param2) {
  ...
  bool effect_changed =
      (old_effect != state.current_effects[active_profile_index]);
  ...
  LOG_DBG("Peripheral: Extracted ug_sync state. Effect=%d, Hue=%d, Layer=%d, "
          "StateDirective=%d",
          state.current_effects[active_profile_index],
          state.colors[active_profile_index].h, layer, state_directive);
...
static int pk_underglow_pm_action(const struct device *dev,
...
DEVICE_DEFINE(pk_underglow_pm, "pk_underglow_pm", pk_underglow_pm_init,
              PM_DEVICE_GET(pk_underglow_pm), NULL, NULL, POST_KERNEL,
