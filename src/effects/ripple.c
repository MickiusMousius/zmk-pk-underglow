...
static void trigger_ripple(uint8_t row, uint8_t col, bool is_rainbow) {
  ...
  ripples[ripple_idx].hue =
      is_rainbow ? global_rainbow_hue : state.colors[active_profile_index].h;
...
static void process_ripples(bool is_rainbow) {
  ...
  for (int i = 0; i < STRIP_NUM_PIXELS; i++) {
    ...
    for (int r = 0; r < MAX_RIPPLES; r++) {
      ...
      uint16_t dist = pk_get_ripple_distance(p_row - ripples[r].row,
                                             p_col - ripples[r].col);
      uint32_t thickness = 200; // 2.0f * 100

      uint32_t dist_from_ring = (current_radius > dist)
                                    ? (current_radius - dist)
                                    : (dist - current_radius);
      ...
      if (dist_from_ring < thickness) {
        ...
        uint8_t ripple_b = (uint8_t)((state.colors[active_profile_index].b *
                                      intensity * fade) /
                                     10000);
