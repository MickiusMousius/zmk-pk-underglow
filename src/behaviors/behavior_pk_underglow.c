...
static int on_keymap_binding_convert_central_state_dependent_params(struct zmk_behavior_binding *binding,
    ...
    case RGB_TOG_CMD: {
        bool state;
        int err = zmk_pk_underglow_get_state(&state);
        if (err) {
    ...
    case RGB_BRI_CMD: {
        struct zmk_led_hsb color = zmk_pk_underglow_calc_brt(1);
    ...
    case RGB_HUI_CMD: {
        struct zmk_led_hsb color = zmk_pk_underglow_calc_hue(1);
    ...
    case RGB_SAI_CMD: {
        struct zmk_led_hsb color = zmk_pk_underglow_calc_sat(1);
    ...
    case RGB_EFR_CMD: {
        ...
        binding->param2 = zmk_pk_underglow_calc_effect(-1);
        break;
    }
    case RGB_EFF_CMD: {
        binding->param1 = RGB_EFS_CMD;
